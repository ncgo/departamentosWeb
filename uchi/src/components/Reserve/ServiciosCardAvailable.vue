...
function roundToNearestPast30(date = new Date()) {
  const minutes = 30;
...
function roundToNearestFuture30(date = new Date()) {
  const minutes = 30;
  const ms = 1000 * 60 * minutes;
...
const checkIfAvailable = () => {
  const availableDates = props.service.fecha_reservada;

  const now = roundToNearestPast30();
  const in30 = roundToNearestFuture30();

  const usedDays = availableDates.map((reservation: any) => {
    return new Date(reservation.date);
  });

  for (let i = 0; i < usedDays.length; i++) {
    if (
      usedDays[i].getTime() == now.getTime() ||
      usedDays[i].getTime() == in30.getTime()
    ) {
...
<template>
  <li class="wrapper">
    ...
    <div v-else="available" class="status false">
